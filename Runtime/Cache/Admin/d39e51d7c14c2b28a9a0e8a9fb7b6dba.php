		...
		<table>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$center): $mod = ($i % 2 );++$i;?><tr>
						...
						<td><?php echo ($center["tel"]); ?></td>
						<td><a href="<?php echo U('index?pid='.$center['id']);?>"><?php echo ($center["title"]); ?></a></td>
						<td><?php echo ($center['type']?'销售':'出租'); ?></td>
                        <td><?php echo ($center["price"]); echo ($center['price_target']?'万元':'元/月'); ?></td>
                        <td><?php echo (date("Y-m-d H:i",$center["create_time"])); ?></td>
                        <td><?php echo ($center["stop_time"]); ?></td>
                        <td><?php echo ($center['status']?'启用':'禁用'); ?></td>
						<td>
							<a title="编辑" href="<?php echo U('edit?id='.$center['id'].'&pid='.$pid);?>">编辑</a>
